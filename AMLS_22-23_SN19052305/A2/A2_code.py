...
def main():
    ...
    #Transform
    transform = transforms.Compose([transforms.Resize((150, 150)), #Resized the image to 150*150 for faster processing
                                    transforms.ToTensor()])
    ...
    # Testing the model
    trained_model_dir = './smile_model.pth'
    ...
    if isFile:
        ...
        for epoch in range(epochs):
            ...
            if early_stopper.early_stop(val_loss):  # Stop when validation loss is not improving enough
                torch.save(model.state_dict(), trained_model_dir)  # Store the model
        ...
        test_accuracy = accuracy_score(test_truth, test_pred)
        print(
            f'Testing loss: {test_loss:.4f} | Testing accuracy: {test_accuracy:.4f}')  # Print testing loss and accuracy
